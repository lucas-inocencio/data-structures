...
class SkipList {
    ...
    insert(x, v) {
        ...
        else { // When we find where to insert x at level 0, apply coin flipping to determine level
    ...
    find(x) {
        ...
        for (let i = this.level; i >= 0; i--) { // else if i > 0, drop to next lower level (i = i-1)
            while (p.forward[i].key < x) { // Walk through level i until finding the rightmost node p such that p.key <= x
