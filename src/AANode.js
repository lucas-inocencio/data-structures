...
class AANode {
    ...
    find(x) {
        let p = this;
        if (p == nil) return null; // unsuccessful search
    ...
    insert(x) {
        ...
        if (p == nil) // fell out of the tree?
            p = new AANode(x, 1, nil, nil); // ... create a new leaf node here
        else if (x < p.key) // x is smaller?
    ...
    kthSmallest(k){
        ...
        let left = p.left.kthSmallest(k);
        if (left != null) return left;
        count++;
        if (count == k) return p;
        return p.right.kthSmallest(k);
    }
}

var nil;
nil = new AANode ('nil', 0, 0);
